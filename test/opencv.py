...
def show(title, im):
	cv2.imwrite("./" + title + "/" + sys.argv[2] + ".jpg", im)
	"""
	cv2.imshow(title,im)
	cv2.waitKey(0)
...
img_c = cv2.imread(sys.argv[1],cv2.IMREAD_COLOR)
img_res = cv2.imread(sys.argv[1],cv2.IMREAD_COLOR)
...
for cnt in contours:
	...
	if area_diff < 0.95:
		continue
	#draw text
	cv2.putText(img_c, str(area_diff) + "---" + str(len(cnt)) + "---" + str(len(approx)) + "----" + str(percentarea), (x,y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 1, cv2.LINE_AA)
	#warp
	print(len(approx))
	if len(approx) != 4:
		continue
